...
- **纯静态**：构建产物是纯 HTML/CSS/JS，可以托管在任何静态空间。仓库里不含任何图片文件，头像和站点图标默认引用自 GitHub；想让页面完全离线自足，把它们换成本地图片即可。
...
**这一步别跳过。** 模板里的头像和站点图标默认指向 `github.com/tzhx12.png`，不换掉的话你的站点会显示别人的头像。

| 改什么 | 在哪 |
| --- | --- |
| **头像、站点图标** | `profile.config.ts` 的 `avatar`、`site.config.ts` 的 `favicon` —— 把地址里的 `tzhx12` 换成你的 GitHub 用户名 |
| 名字、签名、简介、社交链接 | `profile.config.ts` |
| 站点标题、描述、页脚署名 | `site.config.ts` |
| 配色 | `theme.config.ts` 的 `seedColor`，改一个十六进制色值即可 |
| 页面上有哪些模块、各自内容 | `modules.config.ts` |
| 相册图片 | 放进 `public/images/`，在 `modules.config.ts` 的 `gallery` 里引用 |

改完后 `src/config/` 里还会剩一些示例内容（项目、技能、时间线、友链），按同样方式替换成你自己的就行。
...
`theme.config.ts` 的 `background` 段控制页面背景。**仓库里没有附带任何背景图**，默认用的是纯 CSS 光斑，不需要图片就能跑起来：

```ts
background: {
  mode: 'mesh',           // 'mesh' 光斑（默认）/ 'image' 图片+Mica / 'plain' 纯色
  image: '/background.jpg',  // 仅 mode 为 'image' 时生效，需自己提供
...
想用自己的图做 Mica 背景，两步：把图片放进 `public/`，然后把 `mode` 改成 `'image'`、`image` 改成对应文件名。建议宽度 1600px 左右、压在 500KB 以内，原图太大可以压缩：
...
```
public/                  ★ 放你自己的图片资源（仓库默认是空的）
└── images/                  相册等图片放这里，见 images/README.md
